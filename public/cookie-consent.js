(function () {
  ...
  function writeConsent(analytics, marketing) {
    try {
      ...
      window.dispatchEvent(new CustomEvent('luka:cookieConsentChanged', { detail: payload }));
    } catch {}
  }
  ...
  window.lukaCookieConsent = api;
  window.addEventListener('luka:openCookieConsent', showModal);
