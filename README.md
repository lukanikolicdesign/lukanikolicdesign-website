...
Personal UI/UX design portfolio built with Astro.

This is Luka Nikolic's web design portfolio, featuring selected work across websites, e-commerce, dashboards, and design systems.

It includes:

- a polished portfolio homepage
- a dedicated Works page with pagination
- case study pages for Recon, Casper, PrimeStyle, and MV Augusta
...
Update this file when configuring the site:
...
- saved consent in `localStorage` under `luka-cookie-consent`
- a footer `Cookie Preferences` button for reopening the modal
- a `Cookies` policy page at `/cookies`

The site also saves the visitor's color theme in `localStorage` under `luka-theme`.
...
- The consent UI works even if no analytics or marketing tools are connected yet.

### Client API

The consent script exposes `window.lukaCookieConsent` in the browser:

```js
window.lukaCookieConsent.getConsent();
window.lukaCookieConsent.hasConsent();
window.lukaCookieConsent.canUse('analytics');
window.lukaCookieConsent.canUse('marketing');
window.lukaCookieConsent.openPreferences();
```

Whenever a visitor updates their preferences, the site dispatches:

```js
window.addEventListener('luka:cookieConsentChanged', (event) => {
...
```html
<script>
  if (window.lukaCookieConsent?.canUse('analytics')) {
...
The work section contains four completed case studies: Recon, Casper, PrimeStyle, and MV Augusta.
...
- Keep project copy and images aligned with the case studies shown on the site.
- Set `SITE_URL` or `PUBLIC_SITE_URL` before deploying so SEO URLs do not point to the demo domain.
- The social share image is stored at `public/og-image.svg`.
